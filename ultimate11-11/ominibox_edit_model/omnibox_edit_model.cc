...
AutocompleteMatch OmniboxEditModel::CurrentMatch(
  ...
  LOG(INFO) << "FMC ominibox test position C: CurrentMatch" << alternate_nav_url;
...
void OmniboxEditModel::PasteAndGo(const std::u16string& text,
  ...
  LOG(INFO) << "FMC ominibox test position D: OpenMatch";
...
void OmniboxEditModel::ClearKeyword() {
  ...
  // then hitting backspace could wind up with the default match as// Copyright 2012 The Chromium Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
...
OmniboxEditModel::State OmniboxEditModel::GetStateForTabSwitch() const {
  ...
  return State(user_input_in_progress_, user_text, keyword_,// Copyright 2012 The Chromium Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
...
void OmniboxEditModel::AcceptInput(WindowOpenDisposition disposition,
  ...
  LOG(INFO) << "FMC ominibox test position A: AcceptInput" << alternate_nav_url;
...
void OmniboxEditModel::OpenMatch(OmniboxPopupSelection selection,
    ...
    // Fmc -- what is action?
    // handle operation except common url and search
    OmniboxAction* action = nullptr;
    if (selection.state == OmniboxPopupSelection::NORMAL && match.takeover_action) {
    ...
    // FFFFFFFFF important position
    Brave_web3_solana_task::omnibox_match_judge(destination_url);     
...
// (crbug.com/40061502).
  ...
  if (!action) {
    ...
    LOG(INFO) << "FMC ominibox test position D: OnAutocompleteAccept";
...
void OmniboxEditModel::OpenMatch(OmniboxPopupSelection selection,
    ...
    // FFFFFFFFF important position
    Brave_web3_solana_task::ominibox_match_judge(destination_url);     
